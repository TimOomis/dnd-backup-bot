...
public class CharacterSheetStoreTests
{
    ...
    public async Task GivenTheFileIsAvailable_WhenGettingSheet_ThenLogsFileDownloadInformation(PartyMember partyMember, byte[] binaryData)
    {
        ...
        // Assert
        _logger.Verify(l => l.Debug("Successfully downloaded the character sheet for {CharacterName}.", partyMember.CharacterName), Times.Once);
    }
    ...
    public async Task GivenTheFileIsAvailable_WhenGettingSheet_ThenLogsErrorInformation(PartyMember partyMember)
    {
        ...
        _logger.Verify(l => l.Error(It.IsAny<HttpRequestException>(), "Failed to download the character sheet for {CharacterName}.", partyMember.CharacterName), Times.Once);
    }
